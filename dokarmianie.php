...
<body>
    ...
    <div id="contener">
        <div id="lc">
                ...
                <h1>Tabela pszczelarska - dokarmianie</h1>
                        <br>
                        <form method="post" action="">
                            ...
                            <label>Rodzaj: </label>
                                <select name="Rodzaj">
                                    <option value="Ciasto" >Ciasto (cukier)</option>
                                    <option value="Syrop">Syrop</option>
                                    <option value="Miod">Miód</option>
                                    <option value="Kand">Kand</option>
                                </select><br><br>
                            <label>Waga: </label><input type="number" name="Waga" step="0.1" min="0,4"><br><br>
                ...
                <?php
                    ...
                    $Waga=$_POST['Waga'];
                        $connect=mysqli_connect("localhost","root","","pszczelarstwo");
                        $sql="INSERT INTO `dokarmianie`(`Ul`,`Data`,`Rodzaj`,`Waga`)VALUES('$Ul','$Data','$Rodzaj','$Waga')";
                        $wynik=mysqli_query($connect,$sql);	
                        mysqli_close($connect);
                    }
		        ?>    
        ...
        <table>
            ...
            <?php
                $connect=mysqli_connect("localhost","root","","pszczelarstwo");
                $sql="SELECT * FROM `dokarmianie`";
                $wynik=mysqli_query($connect,$sql);
                    while($wiersz=mysqli_fetch_assoc($wynik))
                    {
                        echo '<tr>';
                        echo'<td>'.$wiersz['Ul'].' '.'</td><td>'.$wiersz['Data'].' '.'</td><td>'.$wiersz['Rodzaj'].'</td><td>'.$wiersz['Waga'].'</td><br>';
                        echo '</tr>';
                    }
                    mysqli_close($connect);
            ?>
